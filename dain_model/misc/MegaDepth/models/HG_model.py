...
import dain_model.misc.MegaDepth.pytorch_DIW_scratch as pytorch_DIW_scratch
    ...
    def computeSDR(self, prediction_d, targets):
        ...
        for i in range(0, prediction_d.size(0)):
            ...
            if torch.cuda.is_available():
                ...
                z_B_arr = torch.gather(torch.index_select(predict_depth, 1, x_B_arr.cuda()), 0,
                                       y_B_arr.view(1, -1).cuda())
            else:

                z_A_arr = torch.gather(torch.index_select(predict_depth, 1, x_A_arr), 0, y_A_arr.view(1,
                                                                                                      -1))  # predict_depth:index(2, x_A_arr):gather(1, y_A_arr:view(1, -1))
